...
class Prompt(SoftDeleteMixin):
    section = models.ForeignKey(
        "ReportSection", on_delete=models.CASCADE, related_name="prompts"
    )
    ...
    name = models.CharField(
        max_length=100,
        verbose_name="라벨",
        help_text="프롬프트 이름 (템플릿 섹션의 label)",
    )
...
class PromptTest(models.Model):
    reviewer = models.ForeignKey(
        "Users", on_delete=models.CASCADE, related_name="tests", verbose_name="테스터"
    )
    section = models.ForeignKey(
        "ReportSection", on_delete=models.CASCADE, related_name="tests"
    )
    prompt_text = models.TextField(verbose_name="테스트 프롬프트")
    constraint_snapshot = models.JSONField(
        blank=True, null=True, verbose_name="테스트시점 제약조건"
    )
    ...
    reviewer_comment = models.TextField(
        blank=True, null=True, verbose_name="리뷰코멘트"
    )
