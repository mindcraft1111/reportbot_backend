...
class Projects(ProjectStatusMixin, SoftDeleteMixin):
    ...
    id = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="projects", verbose_name="인덱스"
    )
    project_title = models.CharField(
        max_length=255, null=False, verbose_name="프로젝트명"
    )
    description = models.CharField(
        max_length=255, null=False, verbose_name="프로젝트설명"
    )
    created_at = models.CharField(
        max_length=55, null=False, verbose_name="프로젝트생성일"
    )
    updated_at = models.CharField(
        max_length=55, null=True, verbose_name="프로젝트수정일"
    )
...
class ReportTemplate(SoftDeleteMixin):
    name = models.CharField(
        max_length=200, blank=False, null=False, verbose_name="템플릿명"
    )
...
class Report(ReportStatusMixin, SoftDeleteMixin):
    user = models.ForeignKey(
        Users, on_delete=models.CASCADE, related_name="reports", verbose_name="사용자"
    )
    ...
    template = models.ForeignKey(
        ReportTemplate, on_delete=models.CASCADE, verbose_name="리포트템플릿번호"
    )
    title = models.CharField(
        max_length=200, blank=False, null=False, verbose_name="리포트제목"
    )
    summary = models.CharField(
        max_length=500, blank=True, null=True, verbose_name="리포트요약결과"
    )
...
class ReportSection(SoftDeleteMixin):
    ...
    section_id = models.CharField(
        max_length=100, blank=False, null=False, verbose_name="섹션코드"
    )
    label = models.CharField(
        max_length=200, blank=False, null=False, verbose_name="섹션제목"
    )
...
class ReportSectionResult(ReportStatusMixin, SoftDeleteMixin):
    ...
    constraint_snapshot = models.JSONField(
        blank=True, null=True, verbose_name="생성시점 제약조건"
    )
